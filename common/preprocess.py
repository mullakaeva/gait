...
import pdb
import re
from .utils import fullfile, read_openpose_keypoints, read_and_select_openpose_keypoints, moving_average, OnlineFilter_scalar, OnlineFilter_np
...
def save_data_for_VideoPose3D(save_path, video_size, cut_duration, keypoints_list, records=None):
    ...
    keypoints_list_np = keypoints_list_np.reshape(1, keypoints_list_np.shape[0], keypoints_list_np.shape[1], keypoints_list_np.shape[2] )
    ...
    np.savez(save_path, metadata=metadata, positions_2d=dictionarry_keypoints, vid_info=vid_info, preprocess_info = records)
...
def save_data_openpose(save_path, video_size, cut_duration, keypoints_list, records=None):
    ...
    metadata = {'layout_name': 'body_25', 'num_joints': 25, 'keypoints_symmetry': [[12, 13, 14, 5, 6, 7], [9, 10, 11, 2, 3, 4]]}
    vid_info = {'video_shape': video_size, 'cut_duration':cut_duration}
...
def find_torso_length_from_keyps(keypoints_xy_input, padding):
    ...
    ears_centre_xy = np.mean( keypoints_xy_masked[0:2, [17, 18]], axis = 1)
    neck_xy = keypoints_xy[0:2, 1]
    hip_centre_xy = keypoints_xy[0:2, 8]

    ankle_centre_xy = np.mean( keypoints_xy[0:2, [11, 14]], axis = 1)

    torso_length = np.linalg.norm(ears_centre_xy - neck_xy) + np.linalg.norm(neck_xy - hip_centre_xy) + np.linalg.norm(hip_centre_xy - ankle_centre_xy)
...
class DetectronPreprocessor():
    ...
    def _return_selected_bbox_keyps(boxes_each, keyps_each):
        ...
        def _return_info_by_index(idx):
            ...
            keyps_all_confidence[keyps_all_confidence < 0.01] = -10 # Penalize the confidence of frames with incomplete keypoints confidence
...
class OpenposePreprocessor(VideoManager):
    ...
    def initialize(self):
        ...
        for keyps_path in self.all_kepys_data_paths:
            ...
            selected_centre, selected_keypoints, keyps_confidence = self._return_selected_keypoints(keyps_dict, num_people)
    ...
    def preprocess(self, write_video = True, plot_keypoints=False):
        ...
        for frame_idx, vid_frame in enumerate(self.vreader.nextFrame()):
            ...
            # Dealing with frames that are confident
                ...
                output_frame, keypoints_xy_box_frame, records, box_half_length, central_points = self._find_video_clipping_area(vid_frame, frame_idx)
                ...
                if plot_keypoints:
                    ...
                    fig, ax = plt.subplots()
                    ax.imshow(output_frame)
                    ax.scatter(only_xy[0, :], only_xy[1, :], marker="x", c="r")
    ...
    def _return_selected_keypoints(self, keyps_dict, num_people):
        ...
        def calc_confidence(keyps_selected):
            ...
            keyps_confidence = base_confidence -np.sum(keyps_selected_for_confidence[2,1:15] < 0.1) - np.sum(keyps_selected_for_confidence[2,17:] < 0.1) 
